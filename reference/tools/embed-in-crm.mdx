...
### 4.3 - Map the variable created during the HTTP callout action's creation into the assignment action
...
## Step 6 - Enable a workflow to update the Account in Salesforce
...
- Add a Salesforce `Update` action, using the account ID being passed from the HTTP model to as the `Matching Salesforce Property`