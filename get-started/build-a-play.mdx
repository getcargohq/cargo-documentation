...
<CardGroup cols={2}>
  <Card title="Start with manual testing" icon="flask">
    Always test with manual enrolments before enabling automatic triggers to
    validate your workflow.
  </Card>
  <Card title="Set appropriate re-enrollment rules" icon="rotate">
    Choose the right trigger conditions—use "No Re-Enrollment" for one-time
    actions, "Always Enroll" for repeated processing.
  </Card>
  <Card title="Monitor play health" icon="heart-pulse">
    Set up Slack notifications when error thresholds are hit to catch issues
    early.
  </Card>
  <Card title="Use fallback paths" icon="shield-check">
    Add failure handlers to gracefully manage errors and prevent silent
    failures.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  >
    Use model search, record, and memory nodes in your plays.
  </Card>
